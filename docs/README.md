...
```text
1. 将页面从蓝紫玻璃风格调整为深黑工程控制台风格。
2. 参考 DESIGN.md 的 VoltAgent 视觉语言，使用黑色背景、炭色容器、暖灰边框和绿色信号强调。
3. 收紧卡片圆角到 8px，减少大面积装饰渐变，改用边框和细网格建立层级。
4. 移除内容页无关眉标，仅保留章节编号作为演讲导航。
5. 将翻页控件改为箭头式小按钮，降低对演讲内容的干扰。
```

### 2026-06-09 官网式演示页调整

```text
1. 将内容页改为官网 section 式左右分栏，左侧承载叙事标题，右侧承载产品化展示面板。
2. 将 workflow / method 页面从等宽大卡片改为命令条 + 信号节点 / schema 面板，避免英文关键词超框。
3. 下调分栏页标题字号和卡片字号，提升 1920x1080 演示视口下的稳定性。
4. 针对第二页关键词节点和全 24 页边界做浏览器检查，确认无可视区域超框。
```
